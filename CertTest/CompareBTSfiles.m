...
%--------------------------------------------------        
    ...
    for ic=1:3
        ...
        if exist('dz', 'var')
            for iz = 1:size(dz,3)
                plot(x_2, squeeze( dz(:,ic,iz) ), 'Color', PlotColors(i_color,:) );
                i_color = i_color + 2;
            end
        end
...
%--------------------------------------------------    
    ...
    p1 = zeros(nz_1+nTwr_1,5);
    z1 = [flipud(zTwr_1(:)); z_1(:)];
    
    p2 = zeros(nz_2+nTwr_2,5);
...
%     hubY_2 = [floor(ny_2/2) ceil(ny_2/2)];
    ...
    for i=1:nTwr_1
        ...
        p1(nTwr_1-i+1,3) = mean( squeeze(twrVelocity_1(:,1,i)) );
        p1(nTwr_1-i+1,4) = mean( squeeze(twrVelocity_1(:,2,i)) );
        p1(nTwr_1-i+1,5) = mean( squeeze(twrVelocity_1(:,3,i)) );
    ...
    for i = 1:nz_1
        ...
        p1(i+nTwr_1,3) = mean(mean( squeeze(velocity_1(:,1,hubY_1,i)) ));
        p1(i+nTwr_1,4) = mean(mean( squeeze(velocity_1(:,2,hubY_1,i)) ));
        p1(i+nTwr_1,5) = mean(mean( squeeze(velocity_1(:,3,hubY_1,i)) ));
    ...
    for i=1:nTwr_2
        ...
        p2(nTwr_2-i+1,3) = mean( squeeze(twrVelocity_2(:,1,i)) );
        p2(nTwr_2-i+1,4) = mean( squeeze(twrVelocity_2(:,2,i)) );
        p2(nTwr_2-i+1,5) = mean( squeeze(twrVelocity_2(:,3,i)) );
    ...
    for i = 1:nz_2
        ...
        p2(i+nTwr_2,3) = mean(mean( squeeze(velocity_2(:,1,hubY_2,i)) ));
        p2(i+nTwr_2,4) = mean(mean( squeeze(velocity_2(:,2,hubY_2,i)) ));
        p2(i+nTwr_2,5) = mean(mean( squeeze(velocity_2(:,3,hubY_2,i)) ));
...
%     for i=1:3
%         subplot(1,3,i)
%         plot(p1(:,i+2),z1, '-b+', p2(:,i+2),z2, ':ro');
%         xlabel(['Mean Wind Speed, Component ' num2str(i) ' (m/s)']);
%         ylabel('Height above ground (m)');
%         %title( ['TurbSim Binary Full-Field File Comparisons: ' FileName1 ' and ' FileName2], 'Interpreter','none' );
%         h=legend(FileName1, FileName2);
%         set(h,'Interpreter','none','FontSize',7);
