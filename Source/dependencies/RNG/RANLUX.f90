...
!bjj new      write( RanLux_str, '(I12)' ) JSeed
!bjj new      CALL WrScr( " RanLux default initialization: "//TRIM( ADJUSTL( RanLux_str ) ) )
!end bjj
      LuxLev = LxDflt
      NSkip = NDSkip(LuxLev)
      LP = NSkip + NSeeds - 1
...
!bjj new      write( RanLux_str, '(A,I5,A,I12)' ) " RanLux default luxury level = ", LuxLev, " p = ", LP
!bjj new      CALL WrScr( TRIM( RanLux_str ) )
!end bjj
...
!bjj new       CALL WrScr( " RanLux luxury level set by RLuxIn to: "//TRIM(ADJUSTL(RanLux_str) ))
!end bjj
    else if (LuxLev >= NSeeds - 1) then
      NSkip = LuxLev - NSeeds + 1
!start bjj     
!      print *, " RanLux p-value set by RLuxIn to:", LuxLev
!bjj new      CALL WrScr( " RanLux p-value set by RLuxIn to: "//TRIM(ADJUSTL(RanLux_str) ))
!end bjj
    else
      NSkip = NDSkip(MaxLev)
!start bjj     
...
!bjj new      write (RanLux_str, '(A,I5)') " RanLux luxury level set by RLuxGo :", LuxLev
!bjj new      write (RanLux_str, '(A,I12)') TRIM(RanLux_str)//" p = ", NSkip + NSeeds - 1
!bjj new      CALL WrScr( TRIM(RanLux_str) )
!end bjj
    else
      NSkip = LuxLev - 24
...
!bjj new      write( RanLux_str, '(I20)' ) LuxLev
!bjj new      CALL WrScr( " RanLux p-value set by RLuxGo to: "//TRIM( ADJUSTL(RanLux_str ) ))
!end bjj
    end if
    In24 = 0
    if (Int < 0) then
...
!bjj new      write( RanLux_str, '(3(I12))' ) JSeed, K1, K2
!bjj new      CALL WrScr( " RanLux initialized by RLuxGo from Seeds"//TRIM( RanLux_str ) )
!end bjj
    else
      JSeed = JSDFlt
!start bjj
!      print *, " RanLux initialized by RLuxGo from default seed"
!bjj new      CALL WrScr( " RanLux initialized by RLuxGo from default seed" )
!end bjj
    end if
    InSeed = JSeed
    NotYet = .FALSE.
